 ...
 * @create date 2026-04-13 10:06:24
 * @modify date 2026-04-13 10:06:24
 * @desc [description]
 */
...
export function LogoutConfirmModal({ isOpen, onClose, logoutAction, dictionary }: LogoutConfirmModalProps) {
    ...
    return createPortal(
        ...
        {isOpen && (
            ...
            >
                ...
                <h2 id="logout-modal-title" className="mb-2 text-xl font-semibold text-slate-900 dark:text-white">
                    {dictionary?.logoutConfirmTitle || "End Session?"}
                </h2>
                
                <p className="mb-6 text-sm text-slate-500 dark:text-slate-400">
                    {dictionary?.logoutConfirmMessage || "Are you sure you want to log out of the Eyemate Dashboard? You will need to re-authenticate to access the data."}
                </p>
                ...
                <div className="flex w-full flex-col gap-3 sm:flex-row">
                    ...
                    >
                    {dictionary?.cancelButton || "Cancel"}
                    </button>
                    
                    <form action={logoutAction} className="flex-1">
                    <button
                        type="submit"
                        className="w-full rounded-xl bg-red-600 px-4 py-2.5 text-sm font-semibold text-white shadow-md transition-all hover:bg-red-700 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 active:scale-95 dark:hover:bg-red-500"
                    >
                        {dictionary?.confirmLogoutButton || "Confirm Logout"}
                    </button>
                    </form>
                </div>
                </div>
